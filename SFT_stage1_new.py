...
def save_checkpoint(epoch, step, global_step, model, tokenizer, accelerator, args):
    """
    Save model checkpoint, ensuring correct handling of Hugging Face models
    and DeepSpeed configurations.
    ...
    Args:
        ...
        tokenizer (AutoTokenizer): Tokenizer to be saved.
        accelerator (Accelerator): Hugging Face accelerator for distributed training.
        args (Namespace): Arguments containing paths and training configurations.
    """
    ...
    if accelerator.is_main_process:
        ...
        if not os.path.isdir(args.model_path):
            # Working with Hugging Face Remote Models
            hf_cache_dir = os.path.expanduser(f"~/.cache/huggingface/hub/models--{args.model_path.replace('/', '--')}")
            ...
            if os.path.exists(snapshots_dir) and os.path.isdir(snapshots_dir):
                ...
                snapshot_subdirs = sorted(os.listdir(snapshots_dir), reverse=True)  # Descending by time
                if snapshot_subdirs:
                    latest_snapshot_dir = os.path.join(snapshots_dir, snapshot_subdirs[0])
                    print(f"Using cached model from: {latest_snapshot_dir}")
                else:
            ...
            if latest_snapshot_dir:
                # Copy the files in the snapshot directory
                model_source_dir = latest_snapshot_dir
            # else:
            ...
            #     model_source_dir = hf_cache_dir  # 下载后，直接使用保存的目录
    
        else:
            # 处理本地模型目录
            print(f"Checking local model directory: {args.model_path}")
            model_source_dir = args.model_path
        ...
        print(f'Model checkpoint saved in {output_dir}, copied files: {copy_files}')
    ...
    accelerator.print(f'Checkpoint {save_dir} saved.')
...
def train(args):
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        save_checkpoint(epoch, batch_cnt, global_step, model, tokenizer, accelerator, args)
